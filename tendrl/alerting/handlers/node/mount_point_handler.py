...
class MountPointHandler(AlertHandler):
    ...
    def format_alert_message(self):
        ...
        if self.alert.severity == "INFO":
            self.alert.tags['message'] = "Mount-point utilization of %s in \
            node %s is back to normal. The current utilization is %s %%." % (
        ...
        self.alert.tags['message'] = "Mount-point utilization of %s in \
        node %s is %s %% which is above the %s threshold (%s)." % (
            self.alert.tags['plugin_instance'],
            self.alert.tags['fqdn'],
            str(float(self.alert.current_value)),
            self.alert.severity,
