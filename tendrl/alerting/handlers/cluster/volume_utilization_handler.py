...
class VolumeUtilizationHandler(AlertHandler):
    ...
    def format_alert_message(self):
        ...
        if self.alert.severity == "INFO":
            self.alert.tags['message'] = "Volume utilization of %s in cluster \
            %s is back to normal. The current utilization is %s %%." % (
                self.alert.tags['plugin_instance'],
        ...
        self.alert.tags['message'] = "Volume utilization of %s in cluster %s\
         is %s %% which is above the %s threshold (%s)." % (
            self.alert.tags['plugin_instance'],
            self.alert.tags['cluster_name'],
            str(float(self.alert.current_value)),
            self.alert.severity,
